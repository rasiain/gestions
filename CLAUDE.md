...
**`g_assegurances_polisses`** desa els ajustos manuals per categoria, com `g_taxes_immobles`, amb dos camps que les taxes no necessiten: `inclou`, per a les pòlisses que cap patró no pot enganxar pel nom (`SERVEIS > MUTUALITAT DELS ENGINYERS`), i `companyia`, per unificar l'asseguradora quan l'arbre l'escriu de maneres diferents (CATALANA OCCIDENTE / CATALANA OCCIDENT / OCCIDENT són la mateixa). També hi ha `objecte` (nom del grup, i alhora manera d'unificar dues categories), `poblacio`, `tipus` (etiqueta de la fila, per damunt de la del patró) i `ocult`. L'ajust es busca **tant a la categoria del moviment com al node de la pòlissa**, i es combinen: el municipi se sol desar al node i la companyia a la fulla. L'ajust mana sobre l'arbre.

`g_assegurances_patrons` desa els patrons. Ni els patrons ni els ajustos no tenen pantalla de gestió: s'editen per migració o tinker, com `g_taxes_immobles`.
