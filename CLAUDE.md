...
`/inversions/totals-mobiliaris` (a la navegació, **Anàlisi mobiliari**) reuneix el que les pantalles d'inversions diuen per separat i n'afegeix el que cap no pot dir tota sola: com evoluciona el conjunt, què hi entra i què en surt, i què explica cada salt.

**Què s'hi suma.** Cinc fonts: comptes corrents, fons, plans de pensions, renda fixa i capital social. `PatrimoniMobiliariService` les posa totes en la mateixa forma —un nom, un valor i com es reparteix— i cadascuna es valora **com a la seva pantalla**: el compte pel saldo, el fons i el pla per participacions × darrera cotització, la renda fixa per `valorAData()` (i si no hi ha valor declarat, pel nominal) i el capital social per títols × nominal unitari. Els comptes de tipus `fons_inversio`, `pla_pensions`, `renda_fixa` i `capital_social` **no hi entren com a comptes**: el que valen són els contractes que hi pengen, i comptar-los tots dos els doblaria.
...
L'excepció són **les quotes d'un immoble**. Si el compte és el d'un lloguer i l'immoble té quotes de titularitat desades (`g_propietaris_immobles.quota`), manen elles: Hereus Puigvert Comalada no va a mitges sinó 66,67 % / 33,33 %. Com que les quotes porten `data_inici` i `data_fi`, **poden canviar amb els anys**, i per això cada posició no porta un sol repartiment sinó els seus **trams** (`des_de` + pesos), i la pantalla aplica a cada mes el que tocava aleshores. Sense quotes hi ha un sol tram amb tots els pesos a 1, que és el repartiment de sempre. Un compte que reculli lloguers d'immobles amb quotes diferents no té una resposta única: torna a parts iguals.

Les quotes s'editen a la fitxa de l'immoble, i **una persona hi pot constar més d'un cop**, un cop per tram. Per això els propietaris ja no es desen amb `sync()` —que va indexat per persona i el segon tram esborrava el primer— sinó fila a fila; l'`amortitzacio_anual` del model 184, que no és al formulari, es conserva per (persona, data d'inici).

**La tria** —quins titulars, quines posicions, quin moment i quin rang— **es resol al client**: el servidor ho envia tot i la pantalla filtra, de manera que cada casella que es marca respon sense tornar-hi. Marcar titulars no és només filtrar la suma: **amaga les posicions que no són seves**, perquè la llista de caselles no ofereixi comptes i contractes de qui no s'està mirant.

**La sèrie.** Cada posició porta què valia **a final de cada mes**, des del primer mes amb dades. No cal desar-ho enlloc, perquè les cinc fonts es poden reconstruir: el saldo d'un compte a una data és el `saldo_posterior` del darrer moviment fins aquell dia (llegit amb un cursor de quatre columnes, que hi ha desenes de milers de moviments), i el d'un fons o un pla, les participacions aportades fins aleshores per la cotització vigent. Els mesos sense cap dada arrosseguen el valor del mes anterior. Dues excepcions que el zero no diria bé: **abans de la primera cotització coneguda** s'hi val aquesta mateixa —el contracte hi era encara que la sèrie de valors no arribi tan enrere—, mentre que **abans de comprar un títol de renda fixa** el valor sí que és zero, perquè aleshores no existia. La punta de la sèrie (el mes en curs) dona sempre el mateix que la valoració d'avui, i això és un test.

**L'evolució** n'és la primera gràfica: una barra per període dins d'un rang, amb el total dels titulars marcats en **una sola sèrie**. No es desglossa per titular a posta —per veure'n un, es marca ell sol— i clicar una barra mou el **tall**, que és el moment que desglossa la taula per titular (qualsevol final de mes o d'any, no només avui). Sota la gràfica hi ha els seus mateixos números en una **taula de detall per període**: una fila per barra amb el desglossament per font, la variació contra el període anterior, els fluxos i el mercat. Va del més recent al més antic com la resta de taules del projecte, mentre la gràfica va cap endavant.

**Les notes expliquen els salts** (`g_patrimoni_notes`). Una nota automàtica és **tot moviment d'un compte corrent que passi d'un llindar**, i el criteri és **només l'import**: ni categories ni conceptes, a diferència de les taxes i les assegurances, perquè aquí la pregunta no és de què és el moviment sinó si és prou gros per moure el patrimoni. Els titulars són els del compte del moviment, de manera que una nota només surt a qui afecta. No es desen mai —es dedueixen cada cop, que si no un moviment corregit deixaria enrere una nota que ja no és certa—: el que hi ha desat és el que s'hi afegeix a mà (la `descripcio`, un títol propi, o `ocult` per a un traspàs intern que no explica res) i les notes **manuals**, que no vénen de cap moviment i porten data, títol i titulars propis (`g_patrimoni_nota_titular`; cap titular vol dir «de tothom»).

El servidor n'envia des de `NotesPatrimoniService::MINIM` (1.000 €) i el llindar es mou **al client** amb la resta de la tria (per defecte 3.000 €). D'entrada només s'hi veuen les **despeses**: el que fa baixar el patrimoni és el que se sol buscar, i un cobrament gros no explica cap sotrac; un checkbox hi afegeix els ingressos. Les notes escrites a mà no es filtren mai pel signe —si hi són, és perquè algú les hi ha posat. Cada nota automàtica porta el seu `compte_id`: **desmarcar un compte de la suma també en treu les notes**, perquè un moviment que ja no compta tampoc no explica el que queda. Amb les dades d'avui són 1.424 notes i 315 KB de JSON; si algun dia pesa massa, la palanca és pujar el mínim. Al gràfic són xinxetes damunt de la barra: clicar la barra mou el tall i clicar la xinxeta obre les notes d'aquell període a la taula de detall, que hi diu **quant del salt queda explicat** per les notes que té.

**Els fluxos** (`FluxosPatrimoniService`) són la segona gràfica: quants diners entren i quants en surten cada període, amb el net. Van a part perquè el total és un **estoc** i això són **fluxos**: vuit-cents mil euros de patrimoni no deixarien veure els cinquanta mil d'un mes.

El problema de comptar-los és que **un traspàs entre dos llocs propis no és ni un ingrés ni una despesa**. La tria de la pantalla és una **caixa**: només compta el que en creua la frontera, i com que quins comptes hi ha dins ho decideix la pantalla, el servidor n'envia els **parells** i és ella qui els anul·la. Un parell són dos moviments de comptes diferents amb imports oposats i menys de sis dies de diferència; el cas que més enganya, però, és el que se'n va del compte a un fons, i allà l'altra meitat no és cap moviment —el compte d'un fons no en té— sinó l'**aportació del contracte**, que es busca pel mateix import i amb el mateix marge. Per això els parells van amb claus de posició (`comptes-12` → `fons-3`) i no amb ids de compte. Cada moviment s'aparella un sol cop.

**Anul·lar un traspàs sencer només val si les dues bandes són de la mateixa gent.** Si Joan passa diners a un compte de la Marta, per al patrimoni d'ella allò és una entrada de debò: el que s'anul·la és la part comuna i **el que canvia de mans es compta com el flux que és**. Sense això, amb la Marta i en Ricard marcats el mercat del 2026 sortia +58.011 € en comptes de +29.610 €, perquè 45.500 € que els arribaven d'altres titulars quedaven sense explicar.

**Clicant una barra s'obre el que hi ha darrere**: els moviments del període, repartits, ordenats de més gros a més petit i amb un resum per categoria (en un tall anual són milers de línies i el resum és l'única cosa llegible). Van en un **modal** perquè la pantalla ja té quatre blocs alts, i es demanen **en clicar** (`inversions.moviments-del-tram`) i no amb la pàgina: són milers i la majoria de vegades no es miren. El servidor els torna sencers i sense ordenar —qui els reparteix i els ordena és la pantalla, que és qui sap la tria— i per això cada traspàs porta els **ids dels moviments** que l'han format: si no, el llistat no podria amagar els mateixos que la barra no compta i els totals no quadrarien. Un moviment d'un compte compartit hi surt per la part que toca, amb el sencer a sota.

**El mercat** surt de franc de tenir les dues gràfiques: **variació del total − net dels fluxos**, o sigui el que ha canviat de valor sense que entrés ni sortís cap euro. La taula de detall el dona període a període al costat de la variació. Comprovat per dos camins independents: el 2026 els fluxos donen −63.204 € i el camí de l'estoc −63.182 € (els 22 € són la revaloració del capital social, que és canvi de valor i no cap flux), i el mercat resultant, +29.610 €, coincideix amb sumar posició a posició el que han pujat els fons i els plans descomptant-ne les aportacions.

Sobre un rang sencer, el tram ha de ser el mateix a banda i banda: el punt de partida és **el final del període anterior al rang**, no el del primer període que es veu. Demanant de gener a setembre, es compara amb el 31 de desembre; si es comparés amb el final de gener, el mercat parlaria de febrer-setembre i els fluxos de gener-setembre. Quan el rang arrenca al principi de tot no hi ha res abans i el tram comença al segon període, i l'etiqueta del peu diu sempre quin tram cobreix.

**El mercat és un residu i s'empassa el que no s'expliqui.** Un traspàs intern amb imports diferents (una comissió pel mig) o amb més de sis dies entremig no s'aparella i surt com a despesa; diners que entrin a un fons registrat des d'un compte que no és al sistema el inflen. Si mai cal treure-li aquesta fragilitat, la manera és calcular-lo **directament** —valor menys aportacions, posició a posició— i fer servir el residu com a control: si no dona zero, hi ha un traspàs mal aparellat.
...
- `resources/js/formularis.ts`: `num()` llegeix un número tal com s'escriu aquí (coma decimal, punt de milers) perquè els imports van en camps de **text** —un `type=number` rebutja «100,00» en silenci i deixa el camp buit—, `queFalta()` diu quin camp falta perquè aquests botons de desar **no es deshabiliten mai**, i `perAlServidor()` envia el camp `dia` com a `data`. Usat a capital social, renda fixa i vehicles.
...
┌──────────────────────────────── INVERSIONS ─────────────────────────────────────┐
│                                                                                 │
│  El compte és de tipus fons_inversio / pla_pensions / renda_fixa /              │
│  capital_social i NO té saldo: el que val són els contractes que hi pengen.     │
│                                                                                 │
│  CompteCorrent ──┬── 1:N ──▶ ContracteFons ──────▶ FonsInversio ──▶ ValorFons   │
│   (titulars)     │              └── 1:N ──▶ AportacioFons                       │
│                  ├── 1:N ──▶ ContractePlaPensions ─▶ PlaPensions ─▶ ValorPP     │
│                  │              └── 1:N ──▶ AportacioPlaPensions                │
│                  ├── 1:N ──▶ RendaFixaContracte ──▶ RendaFixaTitol (ISIN)       │
│                  │              ├── 1:N ──▶ RendaFixaValor (patrimonial)        │
│                  │              └── 1:N ──▶ RendaFixaRendibilitat (cupons)      │
│                  └── 1:N ──▶ CapitalSocialContracte                             │
│                                 ├── 1:N ──▶ CapitalSocialValor (títols×nominal  │
│                                 │            o saldo)                           │
│                                 └── 1:N ──▶ CapitalSocialRendiment (amb         │
│                                              retenció)                          │
│                                                                                 │
│  El capital social d'una cooperativa de consum no té compte: porta emissor      │
│  i titulars propis (g_cs_contracte_titular).                                    │
│                                                                                 │
│  Anàlisi mobiliari: PatrimoniMobiliariService (estoc, sèrie mensual)            │
│                     FluxosPatrimoniService (entrades/sortides + traspassos)     │
│                     NotesPatrimoniService (què explica cada salt)               │
└─────────────────────────────────────────────────────────────────────────────────┘
```
