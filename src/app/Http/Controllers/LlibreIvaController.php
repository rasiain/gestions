...
{
    public function desar(Lloguer $lloguer, Request $request): JsonResponse
    {
        ...
        (new Xlsx($this->buildSpreadsheet($lloguer, $any)))->save($rutaFitxer);
    ...
    public function exportar(Lloguer $lloguer, Request $request): StreamedResponse
    {
        $any = $request->integer('any') ?: date('Y');
        $filename = sprintf('llibre-iva-%s-%s.xlsx', $lloguer->acronim ?? $lloguer->id, $any);
        $spreadsheet = $this->buildSpreadsheet($lloguer, $any);
    ...
    private function buildSpreadsheet(Lloguer $lloguer, int $any): Spreadsheet
    {
        ...
        foreach ($factures as $factura) {
            ...
            if ($contracteFra) {
                ...
                if ($llogater) {
                    $llogaterNom = $llogater->nomDisplay();
                    $llogaterNif = $llogater->nifDisplay();
                }
            }

            $dataFormatada = $factura->data_emissio ? $factura->data_emissio->format('d/m/Y') : '';
        ...
        $sheet1->setCellValue("F{$row}", $factures->sum(fn($f) => (float) $f->base));
        $sheet1->setCellValue("H{$row}", $factures->sum(fn($f) => (float) $f->irpf_import));
        $sheet1->setCellValue("J{$row}", $factures->sum(fn($f) => (float) $f->iva_import));
        $sheet1->setCellValue("K{$row}", $factures->sum(fn($f) => (float) $f->total));
        ...
        foreach ($despesesIva as $despesa) {
            ...
            $sheet2->setCellValue("D{$row2}", 100);
        ...
        $sheet2->setCellValue("E{$row2}", $despesesIva->sum(fn($d) => (float) $d->base_imposable));
        $sheet2->setCellValue("G{$row2}", $despesesIva->sum(fn($d) => (float) $d->iva_import));
        ...
        foreach ($despeses as $despesa) {
            ...
            $sheet3->setCellValueExplicit("L{$row}", $proveidor?->codi_postal ?? '', DataType::TYPE_STRING);
